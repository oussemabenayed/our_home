...
// Sample featured properties for fallback - using translation keys
const getSampleProperties = (t) => [
  {
    _id: "sample1",
    title: t('sample_properties.luxury_villa.title'),
    location: t('sample_properties.luxury_villa.location'),
    price: 25000000,
    beds: 4,
    baths: 3,
    sqft: 2800,
  ...
  {
    _id: "sample2",
    title: t('sample_properties.modern_apartment.title'),
    location: t('sample_properties.modern_apartment.location'),
    price: 18500000,
    beds: 3,
    baths: 2,
    sqft: 1800,
  ...
  {
    _id: "sample3",
    title: t('sample_properties.riverside_house.title'),
    location: t('sample_properties.riverside_house.location'),
    price: 12000000,
    beds: 3,
    baths: 2.5,
    sqft: 2200,
      ...
      <div className="relative h-64">
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          <span className="bg-blue-600 text-white text-xs font-medium px-3 py-1.5 rounded-full shadow-md">
            {t(`filters.${property.type.toLowerCase()}`, property.type)}
          </span>
          <span className={`text-xs font-medium px-3 py-1.5 rounded-full shadow-md 
            ${property.availability === 'Rent' || property.availability === 'rent'
              ? 'bg-green-600 text-white' 
              : 'bg-purple-600 text-white'}`}>
            {t('featured_properties.for')} {t(`filters.${property.availability.toLowerCase()}`, property.availability)}
          </span>
        </div>
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center py-3 border-y border-gray-100 mb-4">
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm text-gray-600">{property.beds} {property.beds > 1 ? t('property_card.beds') : t('property_card.bed')}</span>
          </div>
          <div className="flex items-center gap-1">
            <Bath className="w-4 h-4 text-blue-500" />
            <span className="text-sm text-gray-600">{property.baths} {property.baths > 1 ? t('property_card.baths') : t('property_card.bath')}</span>
          </div>
          <div className="flex items-center gap-1">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center text-blue-600 font-bold">
            ...
            <span className="text-xl">{Number(property.price).toLocaleString('en-TN')}</span>
          </div>
          
          <div className="text-sm bg-blue-50 text-blue-700 px-2 py-1 rounded-md flex items-center">
            <Building className="w-3.5 h-3.5 mr-1" />
            {property.availability === 'Rent' ? t('featured_properties.rental') : t('featured_properties.purchase')}
          </div>
        </div>
      </div>
    </motion.div>
...
const PropertiesShow = () => {
  ...
  const categories = [
    { id: 'all', label: t('featured_properties.all_properties') },
    { id: 'apartment', label: t('featured_properties.apartments') },
    { id: 'villa', label: t('featured_properties.villas') },
    { id: 'house', label: t('featured_properties.houses') }
  ];
    ...
    const fetchProperties = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          setError(t('featured_properties.fetch_error'));
          // Fallback to sample data in case of API error
          setProperties(getSampleProperties(t));
        }
      } catch (err) {
        console.error('Error fetching properties:', err);
        setError(t('featured_properties.sample_data_notice'));
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="text-blue-600 font-semibold tracking-wide uppercase text-sm">{t('featured_properties.explore_properties')}</span>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mt-2 mb-4">
            {t('featured_properties.title')}
          </h2>
          <div className="w-24 h-1 bg-blue-600 mx-auto mb-6"></div>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            {t('featured_properties.description')}
          </p>
        </motion.div>
        ...
        {error && (
          ...
          >
            <p className="font-medium mb-1">{t('featured_properties.note')}: {error}</p>
            <p className="text-sm">{t('featured_properties.sample_demo')}</p>
          </motion.div>
        )}
        ...
        {properties.length > 0 ? (
          ...
          <div className="text-center py-10 bg-white rounded-xl shadow-sm">
            ...
            <h3 className="text-xl font-medium text-gray-800 mb-2">{t('featured_properties.no_properties')}</h3>
            <p className="text-gray-600 mb-6">{t('featured_properties.no_properties_category')}</p>
            <button 
              onClick={() => setActiveCategory('all')} 
              className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
            >
              {t('featured_properties.view_all')}
            </button>
          </div>
        )}
        ...
        >
          ...
          >
            {t('featured_properties.browse_all')}
            <ArrowRight className="ml-2 w-4 h-4" />
          </button>
          <p className="text-gray-600 mt-4 text-sm">
            {t('featured_properties.discover_collection')}
          </p>
        </motion.div>
      </div>
    </section>

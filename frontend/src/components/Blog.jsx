...
const BlogCard = ({ post }) => {
  ...
  const handleShare = async (e) => {
    ...
    try {
      if (navigator.share) {
        ...
        toast.success(t('blog.post_shared'));
      } else {
      ...
      toast.error(t('blog.share_error'));
    }
  };
  ...
  const handleBookmark = (e) => {
    ...
    if (!isBookmarked) {
      toast.success(t('blog.saved_to_reading_list', { title: t(post.titleKey) }));
    } else {
      toast.info(t('blog.removed_from_reading_list', { title: t(post.titleKey) }));
    }
  };
  ...
  const estimatedReadTime = Math.ceil(t(post.excerptKey).split(' ').length / 200);
  
  // Extract category from post (or use default)
  const category = t(`blog.categories.${post.category?.toLowerCase()}`) || t('blog.categories.real_estate');
        ...
        <AnimatePresence>
          {isHovered && (
            ...
            >
              ...
              >
                {t('blog.read_full_article')} <ExternalLink className="w-3.5 h-3.5" />
              </button>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="p-6">
        <div className="flex items-center justify-between text-gray-500 text-xs mb-3">
          ...
          <div className="flex items-center">
            ...
            {estimatedReadTime} {t('blog.min_read')}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          ...
          >
            {t('blog.continue_reading')}
            <ChevronRight className="w-4 h-4 ml-1" />
          </button>

          <div className="flex items-center gap-1 text-xs text-gray-500">
            <Tag className="w-3 h-3 text-gray-400" />
            <span>{post.tags?.[0] || t('blog.categories.property')}</span>
          </div>
        </div>
      </div>
    </motion.div>
...
const Blog = () => {
  ...
  const categories = [
    ...
    t('blog.categories.buying'),
    t('blog.categories.selling'),
    t('blog.categories.investment'),
    t('blog.categories.tips')
  ];
  
  const filteredPosts = blogPosts.filter(post => {
    const matchesSearch = t(post.titleKey).toLowerCase().includes(searchTerm.toLowerCase()) || 
                          t(post.excerptKey).toLowerCase().includes(searchTerm.toLowerCase());
    const matchesCategory = selectedCategory === t('blog.categories.all') || 
                           t(`blog.categories.${post.category?.toLowerCase()}`) === selectedCategory;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {filteredPosts.length > 0 ? (
          ...
          >
            ...
            <h3 className="text-xl font-medium text-gray-700 mb-2">{t('blog.no_articles')}</h3>

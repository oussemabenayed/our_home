...
const ScheduleViewing = ({ propertyId, propertyTitle, propertyLocation, propertyImage, onClose }) => {
  ...
  const handleDateChange = (e) => {
    ...
    if (isWeekend(selectedDate)) {
      toast.error(t('schedule.no_weekends'));
      return;
    }
  ...
  const handleTimeChange = (e) => {
    ...
    if (formData.date === dateRestrictions.min && isPastTime(selectedTime)) {
      toast.error(t('schedule.future_time'));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!token) {
        toast.error(t('schedule.login_required'));
        return;
      }
      ...
      const errorMessage = error.response?.data?.message || t('schedule.error');
        ...
        >
          ...
          {!isSuccess ? (
            <>
              ...
              <div className="flex items-center mb-4 pb-3 border-b border-gray-100">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-xl font-bold text-gray-900 truncate">{t('schedule.title')}</h2>
                  {propertyTitle && (
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between">
                  <div className={`flex flex-col items-center ${step >= 1 ? 'text-blue-600' : 'text-gray-400'}`}>
                    ...
                    <span className="text-xs">{t('schedule.date_time')}</span>
                  </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1.5 flex items-center">
                        ...
                        {t('schedule.availability_info')}
                      </p>
                    </div>

                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">
                        {t('schedule.select_time')}
                      </label>
                      <div className="relative">
                        <select
                          value={formData.time}
                        ...
                        >
                          <option value="">{t('schedule.choose_time')}</option>
                          {timeSlots.map((slot) => (
                            <option 
                              key={slot} 
                              value={slot}
                  ...
                  >
                    <div className="bg-blue-50 rounded-lg p-4 mb-4">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="text-sm font-medium text-gray-900">{t('schedule.selected_time')}</h3>
                        <button 
                          type="button" 
                          onClick={() => setStep(1)}
                    ...
                    <div>
                      <label className="flex justify-between text-sm font-medium text-gray-700 mb-1">
                        <span>{t('schedule.additional_notes')}</span>
                        <span className="text-gray-400 text-xs">({t('schedule.optional')})</span>
                      </label>
                      <textarea
            ...
            >
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">{t('schedule.success_title')}</h3>
              <p className="text-gray-600 mb-6">
                {t('schedule.success_message')}
              </p>
          ...
          {!isSuccess && (
            <div className="mt-6 pt-4 border-t border-gray-100">
              <div className="flex items-center text-sm text-gray-600">
                ...
                <span>{t('schedule.agent_guide')}</span>
              </div>
            </div>
          )}
        </motion.div>
